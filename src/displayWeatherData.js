function showSvg(elem, iconID){
    ...
    switch (iconId) {
        ...
        case 'wind':{
            ...
            svg.setAttribute('viewBox','0 0 24 24');
            const path = document.createElementNS('http://www.w3.org/2000/svg','path');
            path.setAttribute('d','M6,6L6.69,6.06C7.32,3.72 9.46,2 12,2A5.5,5.5 0 0,1 17.5,7.5L17.42,8.45C17.88,8.16 18.42,8 19,8A3,3 0 0,1 22,11A3,3 0 0,1 19,14H6A4,4 0 0,1 2,10A4,4 0 0,1 6,6M6,8A2,2 0 0,0 4,10A2,2 0 0,0 6,12H19A1,1 0 0,0 20,11A1,1 0 0,0 19,10H15.5V7.5A3.5,3.5 0 0,0 12,4A3.5,3.5 0 0,0 8.5,7.5V8H6M18,18H4A1,1 0 0,1 3,17A1,1 0 0,1 4,16H18A3,3 0 0,1 21,19A3,3 0 0,1 18,22C17.17,22 16.42,21.66 15.88,21.12C15.5,20.73 15.5,20.1 15.88,19.71C16.27,19.32 16.9,19.32 17.29,19.71C17.47,19.89 17.72,20 18,20A1,1 0 0,0 19,19A1,1 0 0,0 18,18Z');
            svg.appendChild(path);
            elem.appendChild(svg);
            break;
        }
        case 'cloudy':{
            ...
            svg.setAttribute('viewBox','0 0 24 24');
            const path = document.createElementNS('http://www.w3.org/2000/svg','path');
            path.setAttribute('d','M6,19A5,5 0 0,1 1,14A5,5 0 0,1 6,9C7,6.65 9.3,5 12,5C15.43,5 18.24,7.66 18.5,11.03L19,11A4,4 0 0,1 23,15A4,4 0 0,1 19,19H6M19,13H17V12A5,5 0 0,0 12,7C9.5,7 7.45,8.82 7.06,11.19C6.73,11.07 6.37,11 6,11A3,3 0 0,0 3,14A3,3 0 0,0 6,17H19A2,2 0 0,0 21,15A2,2 0 0,0 19,13Z');
            svg.appendChild(path);
            elem.appendChild(svg);
            break;
        }
        ...
        case 'clear-day':{
            ...
            svg.setAttribute('viewBox','0 0 24 24');
            const path = document.createElementNS('http://www.w3.org/2000/svg','path');
            path.setAttribute('d','M12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M12,2L14.39,5.42C13.65,5.15 12.84,5 12,5C11.16,5 10.35,5.15 9.61,5.42L12,2M3.34,7L7.5,6.65C6.9,7.16 6.36,7.78 5.94,8.5C5.5,9.24 5.25,10 5.11,10.79L3.34,7M3.36,17L5.12,13.23C5.26,14 5.53,14.78 5.95,15.5C6.37,16.24 6.91,16.86 7.5,17.37L3.36,17M20.65,7L18.88,10.79C18.74,10 18.47,9.23 18.05,8.5C17.63,7.78 17.1,7.15 16.5,6.64L20.65,7M20.64,17L16.5,17.36C17.09,16.85 17.62,16.22 18.04,15.5C18.46,14.77 18.73,14 18.87,13.21L20.64,17M12,22L9.59,18.56C10.33,18.83 11.14,19 12,19C12.82,19 13.63,18.83 14.37,18.56L12,22Z');
            svg.appendChild(path);
            elem.appendChild(svg);
            break;
        }
...
export function displayWeatherData(response){
    ...
    if(response.days[0].datetime === null){
        locationDateAndTime.textContent = '-';
    } else {
        locationDateAndTime.textContent = response.days[0].datetime;
    }

    // code for the svg
    const introIconHolder = document.querySelector('.intro_icon_holder');
    showSvg(introIconHolder, response.days[0].icon)

    const conditionCardDataValue = document.querySelector('.condition_card>.data_value');
    if(response.days[0].conditions === null){
        conditionCardDataValue.textContent = '-';
    } else {
        conditionCardDataValue.textContent = response.days[0].conditions;
    }

    // code for the svg
    const conditionCardDataIcon = document.querySelector('.condition_card>.data_icon');
    showSvg(conditionCardDataIcon, response.days[0].icon)
    ...
    if(response.days[0].preciptype === null){
        PrecipitationTypeCardDataValue.textContent = '-';
    } else {
        PrecipitationTypeCardDataValue.textContent = response.days[0].preciptype;
    }

    const PrecipitationChanceCardDataValue = document.querySelector('.precipitation_chance_card>.data_value');
    ...
    if(response.days[0].sunrise === null){
        sunriseCardDataValue.textContent = '-';
    } else {
        sunriseCardDataValue.textContent = response.days[0].sunrise;
    }

    const sunsetCardDataValue = document.querySelector('.sunset_card>.data_value');
    if(response.days[0].sunset === null){
        sunsetCardDataValue.textContent = '-';
    } else {
        sunsetCardDataValue.textContent = response.days[0].sunset;
    }
}